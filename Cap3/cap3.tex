...
\begin{equation}
	MSE = ||F - \hat{F}||_F^2 + ||Y - \hat{Y}||_F^2,
\end{equation}

such that:

\begin{gather}
	Z = A_r F \label{eq:z_is}, \\
	\hat{F} = B_r Z \label{eq:f_hat}, \\
	\hat{Y}^T = C_r Z \label{eq:y_hat}.
\end{gather}

Where $A_r \in \mathbb{R}^{2 \times m}$, $B_r \in \mathbb{R}^{m \times 2}$ and $C_r \in \mathbb{R}^{a \times 2}$. $Z \in \mathbb{2 \times n}$ is the matrix of instance coordinates in the IS and $A_r$ is the projection matrix. \cite{Lorena2022} solves this problem numerically using the Broyden–Fletcher–Goldfarb–Shanno (BFGS) algorithm.
...
\begin{figure}[ht]
	\centering
	\begin{tikzpicture}[node distance=3cm and 3cm, align=center, on grid]
		
		% Start node
		\node[draw, rounded corners] (dataset) {Dataset};
		
		% Nodes to the right of dataset
		\node[draw, rounded corners, above right=of dataset] (hardness) {Hardness \\ measures};
		\node[draw, rounded corners, below right=of dataset] (algorithm) {Algorithm \\ space};
		
		% Arrows from dataset to hardness and algorithm
		\draw[->] (dataset.east) -- ++(0.6cm,0) |- (hardness.west);
		\draw[->] (dataset.east) -- ++(0.6cm,0) |- (algorithm.west);
		
		% Node to the right of hardness
		\node[draw, rounded corners, right=of hardness, xshift=2cm] (feature) {Feature \\ selection};
		\draw[->] (hardness.east) -- (feature.west);
		
		% Node to the right of algorithm
		\node[draw, rounded corners, right=of algorithm, xshift=2cm] (performance) {Performance \\ space};
		\draw[->] (algorithm.east) -- (performance.west) node[midway, above] {$y(\alpha, x)$};
		
		% Node to the right of feature and performance
		\node[draw, rounded corners, below right=of feature] (metadata) {Metadata};
		\draw[->] (feature) -- ++(3cm,0) |- (metadata.north);
		\draw[->] (performance) -- ++(3cm,0) |- (metadata.south);
		
		% Node to the right of metadata
		\node[draw, rounded corners, right=of metadata] (instance) {Instance \\ Space};
		\draw[->] (metadata.east) -- (instance.west) node[midway, above] {ISA};
		
	\end{tikzpicture}
	\caption{ISA framework for a single dataset. Extracted from \cite{Lorena2022}.} \label{eq:isa_dataset}
\end{figure}