...
O teste e validação das técnicas de controle propostas foram realizados em um ambiente de simulação e no manipulador
experimental, adquirido através do projeto FAPESP $N^{\circ}$ 98/00649-5, que se encontra em funcionamento no Laboratório de Sistemas Inteligentes (LASI) do Departamento de Engenharia Elétrica da USP em São Carlos. Pode-se citar \cite{Lorena2022}:
\begin{itemize}
\item Isso;
\item Aquilo; e
\item Aquele outro.
...
\section{Motivation}
Manipuladores mecânicos vêm sendo utilizados há várias décadas para a automação de tarefas
...
metodologias de controle tolerante a falhas .
...
liberdade e ajustando-se o sistema de controle adequado, pode-se obter um mecanismo cujo consumo de energia é menor, mas cujas propriedades são mantidas .
